...
import { Create{{ResourceName}}DTO, Update{{ResourceName}}DTO, {{ResourceName}}DTO } from '../dto/{{resourceName}}.dto';
...
export class {{ResourceName}}Service {
  ...
  async findAll(filters?: any): Promise<{{ResourceName}}DTO[]> {
    const entities = await this.{{resourceName}}Repository.findAll(filters);
  ...
  async create(data: Create{{ResourceName}}DTO): Promise<{{ResourceName}}DTO> {
    ...
    // Create entity
    const entity = await this.{{resourceName}}Repository.create({
      ...data,
      createdAt: new Date(),
      updatedAt: new Date()
    });
  ...
  async update(id: string, data: Update{{ResourceName}}DTO): Promise<{{ResourceName}}DTO | null> {
    // Check if exists
    ...
    // Update entity
    const updated = await this.{{resourceName}}Repository.update(id, {
      ...data,
      updatedAt: new Date()
    });
  ...
  private async validateCreate(data: Create{{ResourceName}}DTO): Promise<void> {
  ...
  private async validateUpdate(existing: {{ResourceName}}, data: Update{{ResourceName}}DTO): Promise<void> {
    // Add business validation logic here
  }

  private async validateDelete(existing: {{ResourceName}}): Promise<void> {
